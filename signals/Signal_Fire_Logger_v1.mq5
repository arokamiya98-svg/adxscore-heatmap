//+------------------------------------------------------------------+
//|  Signal_Fire_Logger_v1.mq5                                       |
//|                                                                  |
//|  稼働中の ATR_WidthSignal_v4 の発火を過去再現して全件記録する     |
//|  シグナル検証シートのデータ層（Script 型）。                      |
//|                                                                  |
//|  目的（固定・変更禁止）:                                          |
...
//|    から忠実移植。BT世代2 (gen2) のロジックは参照しない。          |
//|    周期・閾値は v4 の input デフォルト値を「定数」として持つ      |
//|    （input 化禁止。input は走査期間指定のみ）。                   |
//|                                                                  |
//|  v4 との対応（shift 規約）:                                       |
//|    v4 は OnCalculate 内で series 配列 (idx=0 が最新) の各バー i  |
//|    を、そのバー自身の指標値で判定する。本 Logger も全指標を      |
//|    series 配列で一括コピーし、v4 と同一のインデックス演算で      |
//|    判定する（shift 変換を挟まない = ズレの混入経路を断つ）。     |
//|    走査対象は確定バーのみ (i >= 1)。i=0（形成中バー）は判定      |
//|    しない。                                                       |
//|                                                                  |
//|  フィルター F1〜F9:                                               |
//|    「除外せず TRUE/FALSE ラベルのみ」。発火はフィルター前の      |
//|    全件を記録する。TRUE = そのフィルターが抑制対象と判定          |
//|    (= v4 実機なら非表示になる発火)。pass_all = 9本すべて非抑制   |
//|    (= v4 実機で矢印が出た発火)。                                  |
//|    ※共通NG (H1_ADX>40 / ATR_Ratio>2.0) は v4 の発火前提条件     |
//|      (v3bywavelog 継承部分) なのでフィルターではなく「発火なし」 |
//|      として扱う（v4 と同一挙動）。                                |
//|                                                                  |
//|  エントリー基準（C5 原則）:                                       |
//|    entry_price = 発火バーの close（バー確定時点で利用可能な値）。|
//|    指標値も v4 と同じ確定バー基準（v4 の shift 規約から不変）。   |
//|                                                                  |
//|  MFE/MAE:                                                         |
//|    発火バーの次の H1 バーから 48本走査、12/24/36/48h の          |
//|    4セグメント（累積型）。発火方向基準（BUY発火なら BUY視点）。  |
//|    参照: XAUUSD_Daily_MFE_MAE_v1.mq5 TraceMaeMfe_Segmented       |
//|                                                                  |
//|  出力: MQL5/Files/signal_fires.csv (UTF-8 BOM / 全量上書き)      |
//|  JST/DST 変換: Trade_Snapshot_Builder.mq5 と同じ規約             |
//|    (TimeTradeServer-TimeGMT 自動算出。過去日の DST 跨ぎは        |
//|     実行時オフセット依存 → time_server 列が厳密値)               |
//|                                                                  |
...
const string OUTPUT_FILE        = "signal_fires.csv";
...
void OnStart()
{
   Print("==== Signal_Fire_Logger v1.0 Start ====");
   ...
   //--- 指標ハンドル生成（v4 L262-274 と同一構成。H4 MA は判定未使用のため省略）---
   int hATR_S_H1 = iATR(_Symbol, PERIOD_H1, H1_ATR_SHORT);
   int hATR_L_H1 = iATR(_Symbol, PERIOD_H1, H1_ATR_LONG);
   int hADX_H1   = iADX(_Symbol, PERIOD_H1, H1_ADX_PERIOD);
   int hMA_H1    = iMA (_Symbol, PERIOD_H1, H1_MA_PERIOD, 0, MODE_EMA, PRICE_CLOSE);
   int hATR_S_H4 = iATR(_Symbol, PERIOD_H4, H4_ATR_SHORT);
   int hATR_L_H4 = iATR(_Symbol, PERIOD_H4, H4_ATR_LONG);
   int hADX_H4   = iADX(_Symbol, PERIOD_H4, H4_ADX_PERIOD);
   int hATR_S_D1 = iATR(_Symbol, PERIOD_D1, D1_ATR_SHORT);
   int hATR_L_D1 = iATR(_Symbol, PERIOD_D1, D1_ATR_LONG);
   int hADX_D1   = iADX(_Symbol, PERIOD_D1, D1_ADX_PERIOD);
   ...
   {
      Print("[FATAL] 指標ハンドル生成失敗");
      return;
   }
   Sleep(3000);  // インジ計算待ち（Script なので明示待機）
   ...
   if(start_idx < 0)
   {
      ...
      ReleaseAll(hATR_S_H1, hATR_L_H1, hADX_H1, hMA_H1,
                 hATR_S_H4, hATR_L_H4, hADX_H4,
                 hATR_S_D1, hATR_L_D1, hADX_D1);
      return;
   }
   ...
   bool copy_ok = true;
   copy_ok = copy_ok && (CopyBuffer(hATR_S_H1, 0, 0, copy_size, atr_s_h1) > 0);
   copy_ok = copy_ok && (CopyBuffer(hATR_L_H1, 0, 0, copy_size, atr_l_h1) > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_H1,   0, 0, copy_size, adx_h1)   > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_H1,   1, 0, copy_size, dip_h1)   > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_H1,   2, 0, copy_size, din_h1)   > 0);
   copy_ok = copy_ok && (CopyBuffer(hMA_H1,    0, 0, copy_size, ma_h1)    > 0);
   ...
   if(!copy_ok)
   {
      PrintFormat("[FATAL] H1 データ取得失敗 err=%d (要求 %d 本。チャート設定の"
                  "「最大バー数」を確認)", GetLastError(), copy_size);
      ReleaseAll(hATR_S_H1, hATR_L_H1, hADX_H1, hMA_H1,
                 hATR_S_H4, hATR_L_H4, hADX_H4,
                 hATR_S_D1, hATR_L_D1, hADX_D1);
      return;
   ...
   copy_ok = true;
   copy_ok = copy_ok && (CopyBuffer(hATR_S_H4, 0, 0, h4_copy_size, atr_s_h4) > 0);
   copy_ok = copy_ok && (CopyBuffer(hATR_L_H4, 0, 0, h4_copy_size, atr_l_h4) > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_H4,   0, 0, h4_copy_size, adx_h4)   > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_H4,   1, 0, h4_copy_size, dip_h4)   > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_H4,   2, 0, h4_copy_size, din_h4)   > 0);
   copy_ok = copy_ok && (CopyTime(_Symbol, PERIOD_H4, 0, h4_copy_size, h4_time) > 0);
   if(!copy_ok)
   {
      PrintFormat("[FATAL] H4 データ取得失敗 err=%d", GetLastError());
      ReleaseAll(hATR_S_H1, hATR_L_H1, hADX_H1, hMA_H1,
                 hATR_S_H4, hATR_L_H4, hADX_H4,
                 hATR_S_D1, hATR_L_D1, hADX_D1);
      return;
   ...
   copy_ok = true;
   copy_ok = copy_ok && (CopyBuffer(hATR_S_D1, 0, 0, d1_copy_size, atr_s_d1) > 0);
   copy_ok = copy_ok && (CopyBuffer(hATR_L_D1, 0, 0, d1_copy_size, atr_l_d1) > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_D1,   0, 0, d1_copy_size, adx_d1)   > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_D1,   1, 0, d1_copy_size, dip_d1)   > 0);
   copy_ok = copy_ok && (CopyBuffer(hADX_D1,   2, 0, d1_copy_size, din_d1)   > 0);
   copy_ok = copy_ok && (CopyTime(_Symbol, PERIOD_D1, 0, d1_copy_size, d1_time) > 0);
   if(!copy_ok)
   {
      PrintFormat("[FATAL] D1 データ取得失敗 err=%d", GetLastError());
      ReleaseAll(hATR_S_H1, hATR_L_H1, hADX_H1, hMA_H1,
                 hATR_S_H4, hATR_L_H4, hADX_H4,
                 hATR_S_D1, hATR_L_D1, hADX_D1);
      return;
   ...
   Print("==== Signal_Fire_Logger v1.0 Complete ====");
...
//| ReleaseAll                                                       |
//+==================================================================+
void ReleaseAll(int h1, int h2, int h3, int h4,
                int h5, int h6, int h7,
                int h8, int h9, int h10)
{
   ...
   if(h6  != INVALID_HANDLE) IndicatorRelease(h6);
   if(h7  != INVALID_HANDLE) IndicatorRelease(h7);
   if(h8  != INVALID_HANDLE) IndicatorRelease(h8);
   if(h9  != INVALID_HANDLE) IndicatorRelease(h9);
   if(h10 != INVALID_HANDLE) IndicatorRelease(h10);
}
//+------------------------------------------------------------------+
