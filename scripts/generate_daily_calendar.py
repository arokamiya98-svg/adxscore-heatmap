...
<title>Daily Research Calendar — マニ v2.0</title>
<style>
...
.cell {
  border-radius: 4px;
  display: grid;
  grid-template-rows: 16px 1fr 22px 4px 20px 28px;  /* 日付 / H4主役 / H1補助 / 区切り / シグナル / 結果 */
  min-height: 160px;
  position: relative;
  background: #05090f;
...
html.append(f'<h1>DAILY RESEARCH CALENDAR — マニ v2.0</h1>')
virtual_count = len(daily_mfe_mae_map)
daily_agg_count = len(daily_agg_map)
html.append(f'<div class="sub">入力: {src.name} / 日次集計: {daily_agg_count}日 / 仮想MFE/MAE: {virtual_count}日 / 期間: {all_dates[0]:%Y-%m-%d} 〜 {all_dates[-1]:%Y-%m-%d} / 生成: {datetime.now():%Y-%m-%d %H:%M} / 全体比正規化基準(p95): {BAR_NORM_BASE:.0f} USD / 最大観測値: {BAR_MAX_OBS:.0f} USD</div>')
    ...
    for week_days in weeks_list:
        ...
        for d in week_days:
            ...
            if trades:
                ...
                # v0.8: スコアを右上に小フォントで残す（ADX生値は tooltip 参照）
                html.append(f'<span class="adx-tiny">SC{score_txt}</span>')
                ...
                if virtual and (virtual["buy_mfe"] is not None or virtual["sell_mfe"] is not None):
                    # 右上スコア
                    html.append(f'<span class="adx-tiny" style="opacity:0.7;">SC{score_txt}</span>')
                    # 左上「仮」マーク（仮想エントリーの明示）
                    ...
                    # データ欠損日（CSV期間外、休場日、データ未取得）は従来通り中央クリーン
                    html.append(f'<span class="adx-tiny" style="opacity:0.7;">SC{score_txt}</span>')
