...
─────────────────────────────
日次認識カレンダー v3 移植版（v2ベース + シグナル統合 / Step 1）

指示書: data/mani_room/マニ_指示書_日次認識カレンダーv3移植版_v0.2.md
...
html.append(f'<div class="sub">入力: {src.name} / 日次集計: {daily_agg_count}日 / 仮想MFE/MAE: {virtual_count}日 / 期間: {all_dates[0]:%Y-%m-%d} 〜 {all_dates[-1]:%Y-%m-%d} / 生成: {datetime.now():%Y-%m-%d %H:%M} / 全体比正規化基準(p95): {BAR_NORM_BASE:.0f} USD / 最大観測値: {BAR_MAX_OBS:.0f} USD'
            f' / シグナル発火: {n_fires_total}件 (pass {n_fires_pass} / 抑制 {n_fires_supp}) {all_fire_cell_dates[0]:%Y-%m-%d}〜{all_fire_cell_dates[-1]:%Y-%m-%d}</div>')
...
html.append('<span style="font-size:11px;opacity:0.3;color:#FFD700;">▲</span>'
            '<span style="font-size:9px;">pass_all=FALSE（フィルター抑制 = 実機では見えなかった発火）</span>')
...
for m_start in month_iter(start, end):
    ...
    for week_days in weeks_list:
        ...
        for d in week_days:
            ...
            html.append('<div class="fires-row">')
            for fr in day_fires:
