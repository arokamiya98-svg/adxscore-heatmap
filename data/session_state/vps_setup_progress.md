...
## 別荘 安定化メモ
- 2GB実態（2026-06-18判明）: 圧迫主犯は**Claude Code自身**(約2.5GB)、MT5は軽い(162MB)。EA無人化は2Gで可、Win2(3.5GB)はおぱ常駐の快適性用（今すぐ不要）
- **作業時はMT5を一時的に閉じる**（メモリ確保）→ **作業後にMT5を開いて軸A再稼働**（閉じてる間は24h通知停止に注意）
- RDPは「**切断**」で抜ける（**ログオフ厳禁**＝MT5/Claudeアプリが落ちる）
- 作業の節目で push 癖をつける（落ちる前提の保険／push前は `git pull --rebase`）
