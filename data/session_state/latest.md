...
1. **まず `git pull --rebase origin main`** — VPSとMacは同じmainを書き合うパラレル運用。起動直後にpullしないと状況が古い（VPSが自動pushした `mt5_data/daily/` も、この latest.md 自体も最新化されない）。
2. この latest.md の「**今ここ**」「**次の起点**」で現状とアクションを把握。詳細は設計書 `data/vps/日次動脈_DESIGN_v1.md`。
3. **push前も必ず `git pull --rebase`**。RDPは「切断」で抜ける（ログオフ厳禁）。
4. 迷ったら「今どのフェーズ？」を latest.md で確認 → 該当フェーズの手順を踏む。
