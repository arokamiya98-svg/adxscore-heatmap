//---------------------------------------------------
// Scriptable ATR Ratio 基準線 Widget v1
// XAUUSD 認識ツール（価格非依存・ATR Ratio の $基準線 3本）
//
// 仕様: data/scriptable/SPEC_ratio_widget_v1.md
// 根拠: data/bt/PATTERN_REGIME_MAP_v2_AtrRatioEdge.md（0.7/1.2 の由来）
// 姉妹: data/scriptable/atr_widget.js（既存＝加熱ウィジェット。本ファイルは一切それを触らない）
//
// 役割:
//   ATR Ratio の「基準線」を $絶対値 で常時表示する。
//   中央値(Ratio 1.0) / 0.7値(収束下限) / 1.2値(拡張) の 3つの $基準値 を出す。
//   ＋SL/TP 物差し行（B1確定式・2026-07-10 追加。物差しでありルールではない）。
//
// ★重大原則（あろさん・認識ツール思想）:
...
// 寸分同じ定義で組む。カイのBTが炙り出した 0.7/1.2 ゾーンをそのまま現場の $ライン化する。
//---------------------------------------------------
...
const CONFIG = {
  ...
  // ATR 系列の周期（BT: H1_ATR_Short=16）
  atr_period: 16,

  // 中央値の対象本数（BT: ATR_Median_Weeks(8) * 5営業日 * 24h = 960）
  // 履歴が満たなければ「取れた分」で算出（calcMedianBT 側でフォールバック）。
  median_bars: 8 * 5 * 24,   // = 960

  // Ratio 基準（BT の 0.7/1.2 ゾーン。中央値=1.0 に対する倍率）
  ratio_low:  0.7,   // 収束下限
  ratio_high: 1.2,   // 拡張

  // SL/TP 物差し（B1確定: data/bt/SPEC_new_BT.md §2。SL=ATR_Avg32×2.0 / TP=SL×1.6）
  // 単位はドル距離のまま整数表示（「SL60」の数え方。SPEC_new_BT の ×100 pips 換算は使わない）
  sltp_avg_bars: 32,   // ATR_Avg32 = 直近32本の ATR(16) 単純平均
  sl_mult: 2.0,
  tp_mult: 1.6,

  // 取得本数。UTC土日フィルタ後に median_bars(960) を確保する余裕を持たせる。
...
// キャッシュは専用ファイルに分離（既存 atr_widget の state/cache とは混ぜない）。
const FM = FileManager.local();
const CACHE_PATH = FM.joinPath(FM.documentsDirectory(), "ratio_widget_cache.json");
...
function calcMedianBT(series) {
  ...
  // 直近 median_bars 本（末尾960。無ければある分）。BT series=true 窓 [idx, idx+bars) に相当。
...
function buildWidget(data) {
  ...
  const title = header.addText("XAUUSD ATR基準 H1");
  ...
  // ----- 基準線 3行（方向ラベルなし・値幅局面色のみ）+ SL/TP 物差し行 -----
  ...
  addSlTpRow(widget, data.atr_avg32);
...
// SL/TP 物差し行（B1確定式: SL = ATR_Avg32×2.0 / TP = SL×1.6、ドル距離を整数表示）。
// 物差しでありルールではない → 基準線3行と区別してラベルは DIM、値幅局面色も付けない。
function addSlTpRow(widget, atrAvg32) {
  const row = widget.addStack();
  row.layoutHorizontally();
  row.centerAlignContent();

  const labelText = row.addText("SL/TP");
  labelText.font = Font.systemFont(11);
  labelText.textColor = COLOR_DIM;

  row.addSpacer();

  let valueStr = "N/A";
  if (atrAvg32 != null) {
    const sl = atrAvg32 * CONFIG.sl_mult;
    const tp = sl * CONFIG.tp_mult;
    valueStr = `${Math.round(sl)} / ${Math.round(tp)}`;
  }
  const valueText = row.addText(valueStr);
  valueText.font = new Font("Menlo", 14);
...
async function main() {
  ...
  try {
    // H1 を1 call 取得（outputsize=1600）。土日フィルタ後に ATR(16) 系列 → 直近960本の中央値。
    const h1 = await fetchOHLC("1h", CONFIG.outputsize);
    const series = calculateWilderATRSeries(h1, CONFIG.atr_period);
    const median = calcMedianBT(series);   // 直近960本（無ければある分）/ 有効<10 は null

    // SL/TP 物差し用: ATR_Avg32 = 直近32本の ATR(16) 単純平均（32本未満は null → N/A）
    const atrAvg32 = (Array.isArray(series) && series.length >= CONFIG.sltp_avg_bars)
      ? series.slice(-CONFIG.sltp_avg_bars).reduce((a, b) => a + b, 0) / CONFIG.sltp_avg_bars
      : null;

    if (median == null) {
      // 履歴が浅く中央値が出せない → N/A（画面は殺さない）
      data = { median: null, v07: null, v12: null, atr_avg32: atrAvg32, error: "履歴不足（N/A）" };
    } else {
      data = {
        median: median,
    ...
    if (cached) {
      data = {
        ...
        v07: cached.v07,
        v12: cached.v12,
        atr_avg32: cached.atr_avg32 ?? null,  // 旧キャッシュにキー無し → N/A
        error: "API Err（キャッシュ表示）",
      };
    } else {
      data = { median: null, v07: null, v12: null, atr_avg32: null, error: "Loading..." };
    }
  }
