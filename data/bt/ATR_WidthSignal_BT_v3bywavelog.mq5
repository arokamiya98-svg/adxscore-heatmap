//+------------------------------------------------------------------+
//|  ATR_WidthSignal_BT_v3bywavelog.mq5                              |
//|  新規BT: 5パターン × BUY/SELL × multi-fire フラット記録          |
//|                                                                  |
//|  仕様: ADXSCORE/data/bt/SPEC_new_BT.md                          |
//|  ベース: signals/ATR_WidthSignal_v3bywavelog.mq5                |
//|                                                                  |
//|  実行: 任意チャートにスクリプトをドロップ → 入力確認 → 実行     |
//|  出力: MT5/MQL5/Files/ATR_WidthSignal_BT_NEW.csv (UTF-16 LE BOM) |
//|                                                                  |
//|  原則: 結果フィッティング禁止・全件記録 (memory:bt-analysis-...)|
//+------------------------------------------------------------------+
...
input string   BT_OutputFile   = "ADXSCORE_bt\\ATR_WidthSignal_BT_NEW.csv";
...
void OnStart()
{
   Print("==== ATR_WidthSignal_BT_v3bywavelog Start ====");
   ...
   FileHandle = FileOpen(BT_OutputFile, FILE_WRITE|FILE_TXT|FILE_UNICODE, ',');
   ...
   PrintFormat("Output: %s/MQL5/Files/%s", TerminalInfoString(TERMINAL_DATA_PATH), BT_OutputFile);
